...
import { Ajv2020 as Ajv, type ValidateFunction as AjvValidateFunction } from 'ajv/dist/2020';
...
export function compile( args: CompileOptions ): void {
    ...
    const target = args.target? args.target : path.format({ ...path.parse(args.schema), base: '', ext: '.validator.js' });
    ...
    fs.writeFileSync(path.join(path.dirname(target), path.basename(target, '.js') + '.d.ts'), dts);
}
