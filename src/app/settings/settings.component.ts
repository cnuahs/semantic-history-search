...
export class SettingsComponent implements OnInit {
  ...
  constructor( private router: Router, private settingsService: SettingsService, private formBuilder: FormBuilder) {
        ...
        (settings: Setting[]) => {
          ...
          this.settings?.forEach((setting) => {
            if (Array.isArray(setting.value)) {
              this.form.addControl(setting.name, this.formBuilder.array(setting.value.map((value) => this.formBuilder.control(value))));
            } else {
